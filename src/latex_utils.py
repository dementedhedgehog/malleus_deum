...
def find_xelatex(verbosity=0):
    ...
    # sanity check
    assert exists(executable), f"Can't find xelatex at {executable}"
    if verbosity > 0:
        print("Using xelatex at %s" % executable)
...
def find_makeindex(verbosity=0):
    """
    Return the path to the makeindex executable (a latex tool), or die!
...
def filter_xelatex_output(xelatex_output):
    """
    Filter out some noisy common latex error logs that are not important.
...
def build_pdf(
    ...
    # filenames
    doc_basename, _ = splitext(basename(xml_fname))
    pdf_fname = join(build_dir, f"{doc_basename}.pdf")
    tex_fname = join(build_dir, f"{doc_basename}.tex")
    short_idx_fname = "%s.idx" % doc_basename
    idx_fname = join(build_dir, short_idx_fname)
    ...
    if not fast_mode:
        with codecs.open(tex_fname, "w", "utf-8") as f:           
            ...
            if len(errors) > 0:
                ...
                for error in errors:
                    str_rep += "\t%s\n" % error
                raise Exception(str_rep)
                
    # Don't build the pdf files?
    ...
    if return_code != 0:
        #sys.exit(f"\tFailed to run makeindex on %s\n" % idx_fname)
        raise Exception(f"Failed to run makeindex on %s\n" % idx_fname)

    # rerun latex to build the pdf with the up to date index
    if not xelatex(tex_fname, verbosity=verbosity):
        raise Exception(f"Failed to build {pdf_fname}\n")
