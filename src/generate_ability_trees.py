...
"""
  ...
  Draws Skill/Ability Trees.   
   - these are the eps diagrams that show skill prereqs
     (e.g. a bit like tech trees but for pcs).
...
# The font has to installed to the system for pycairo to find it.
# If pycairo can't find the font it silently loads a default.
FONT_SIZE = 8
FONT_FACE = "Sherwood"

ABILITY_MARGIN_TOP = 5
...
class AbilityNode:
    ...
    def draw(self, context, starting_pos, has_prereqs=True):
        x, y = starting_pos
        ...
        # draw ability label
        context.move_to(
            x0 + ABILITY_MARGIN_LEFT,
            y0 + half_height + ABILITY_MARGIN_TOP)
        context.show_text(title)
        
        # draw the little ball on middle top that represents the incoming pos
        # if we're not the origin node.
        if has_prereqs:
            black_ball_radius = 2
    ...
    def __str__(self):
        return self.ability.get_id()


def draw_ability_tree(
        ability_groups,
        ability_group,
        ability,
        context,
    ...
    Calls itself recursively visiting each node in the tree using the visitor
    pattern.
    ...
    starting_pos, new_max_y = node.draw(
        context,
        starting_pos,
        has_prereqs=ability.has_prerequisites())
...
def build_ability_trees(ability_groups):
    for ability_group in ability_groups: 
        ...
        # height.
        max_y = _build_ability_tree(ability_groups, ability_group, fname=fname)

        # Second time we know the height and make the image the right size
        _build_ability_tree(
            ability_groups,
            ability_group,
            fname=fname,
