...
# of <flat13/> type elements.
_flatcheck_regex = re.compile(r'(\d+)$')
...
%%\usepackage{pgfornament}         %% Page and table decorations.
...
\AddToHook{shipout/background}{%%
  ...
  %%   %%\getpagebackground{}
       \put (0pt,-\paperheight) {\transparent{0.1}\includegraphics[width=\paperwidth,height=\paperheight]{./resources/anon_elder_sign/anon_elder_sign.png}}
  \fi

  \ifnum\value{modpagenumber}=2\relax
  %%   %%\getpagebackground{}
       \put (0pt,-\paperheight) {\transparent{0.1}\includegraphics[width=\paperwidth,height=\paperheight]{./resources/mj_pigman/mj_pigman.png}}
  \fi

  \ifnum\value{modpagenumber}=3\relax
  %%   %%\getpagebackground{}
       \put (0pt,-\paperheight) {\transparent{0.1}\includegraphics[width=\paperwidth,height=\paperheight]{./resources/mj_squid_experiment/mj_squid_experiment.png}}
  \fi

  \ifnum\value{modpagenumber}=4\relax
  %%   %%\getpagebackground{}
       \put (0pt,-\paperheight) {\transparent{0.1}\includegraphics[width=\paperwidth,height=\paperheight]{./frame_test.png}}
  \fi

  \ifnum\value{modpagenumber}=5\relax
  %%   %%\getpagebackground{}
       \put (0pt,-\paperheight) {\transparent{0.1}\includegraphics[width=\paperwidth,height=\paperheight]{./resources/dragon_title/dragon.png}}
  \fi
  ...
  \ifnum\value{modpagenumber}=7\relax
  %%   %%\getpagebackground{}
...
%%\newfontfamily{\caudex}[Path=fonts/Caudex, Scale=1.1]{Caudex}
%%\newfontfamily{\becker}[Path=fonts/]{Becker Regular}

%%
%% Font Aliases
%%
%%\newfontfamily{\quotefont}[Path=fonts/, Scale=1.4]{Isabella}
%%\newfontfamily{\quotefont}[Path=fonts/, Scale=1.4]{Isabella}
...
%%\setmainfont[Scale=0.95]{Becker-ZVrz.ttf}
%%\setromanfont[
%%  Mapping=tex-text, 
%%  Mapping=tex-text, 
%%  Ligatures={Common,Rare,Discretionary}]{Becker Regular}
\setmainfont[
  %%Scale=0.95,
  Path = ./fonts/Caudex/,
  UprightFont = {*-Regular},
...
%%  Mapping=tex-text, 
%%  Mapping=tex-text, 
%%  Ligatures={Common,Rare,Discretionary}]{Caudex}
...
\lettrine[%%
 ...
 loversize=0.2, %%
 slope=0em]%%
{\dropcapfont\color{dropcapcolor}#1}{#2}}
...
%% Antag Check Symbol
\newcommand\antagonistchecksymbol{%%
\raisebox{\symbolverticaloffset}{%%
\includegraphics[width=\symbolsize,height=\symbolsize]%%
...
# Decorators for shortcuts.
# def tag(formatter, fn):
#     setattr(formatter, "start_" + fn.__name__,  fn)
#     setattr(formatter, "end_" + fn.__name__,  formatter.no_op)
...
class LatexFormatter:
    ...
    def get_buffer_str(self, strip=False):
    ...
    def start_antagonistchecksymbol(self, symbol):
        self.buffer.write(r"\antagonistchecksymbol ")
        return
    end_antagonistchecksymbol = no_op    
    ...
    def start_corollary(self, symbol):
        self.buffer.write("\\begin{corollary}")
        return
    
    def end_corollary(self, symbol):
        self.buffer.write("\\end{corollary}")
        return
    ...
    def start_principle(self, symbol):
        self.buffer.write("\\begin{principle}")
        return
    
    def end_principle(self, symbol):
        self.buffer.write("\\end{principle}")
        return    
    ...
    def start_sidebartitle(self, section_title):
        self.buffer.write(r"\begin{large}\begin{embolden}")
        return
    def end_sidebartitle(self, section_title):
        self.buffer.write(r"\end{embolden}\end{large}\vspace{0.5em}")
        return    
    ...
    def start_titlepage(self, chapter):
        ...
        # Experimental page ornaments
#         self.buffer.write(
#             r"""
# \begin{tikzpicture}[remember picture, overlay]            
# \node[anchor=north west] at (current page.north west)
# {\pgfornament[width=1cm]{31}};
# \node[anchor=north east] at (current page.north east)
# {\pgfornament[width=1cm,symmetry=v]{31}};
# \node[anchor=south west] at (current page.south west)
# {\pgfornament[width=1cm,symmetry=h]{31}};
# \node[anchor=south east] at (current page.south east)
# {\pgfornament[width=1cm,symmetry=c]{31}};
# \end{tikzpicture}
#             """)
        return
    ...
    def start_bold(self, bold):
        self.buffer.write("\\textbf{%s}" % normalize_ws(bold.text))
        return
    end_bold = no_op
    ...
    def write_index(self, entry, subentry, see, definition):
        ...
        if defn:
            ...
            # The intent is to write short descriptions of concepts
            # in the index and save people having to actually look up pages.
            # (do not mess with these lines ... they are deep latex magic!)
            index_str += (r"!aaaaaaaa@\empty \igobble "
                          r"|seealso{\hspace{-2ex}%s}" % defn)
        index_str += "}"
        self.buffer.write(index_str)
        return
    ...
    def start_indent(self, indent_subentry):
        self.buffer.write(r"\begin{mdindent}")        
    def end_indent(self, indent_entry):
        self.buffer.write(r"\end{mdindent}")
        
    def start_quote(self, indent_subentry):
        self.buffer.write(r"\begin{mdquote}")
        
    def end_quote(self, indent_entry):
    ...
    def end_indexdefn(self, index_defn):
        ...
        self.index_entry_definition = self.get_buffer_str(strip=True)
        return
    ...
    def start_defn(self, defn):
        #self.buffer.write(r"\begin{embolden}")
        self.buffer.write(r"\begin{defn}")
        return
    def end_defn(self, defn):
        #self.buffer.write(r"\end{embolden}")
    ...
    def start_img(self, img):        
        ...
        self.buffer.write("\t\\includegraphics[%s]{%s}\n"
                          % (img_size, filename))
        return
    ...
    def start_table(self, table):
        ...
        if tabcolsep is not None:
            self.buffer.write(
                r"\setlength{\tabcolsep}{%s\tabcolsep}" %
                tabcolsep)
    ...
    def end_table(self, table):
        ...
        for index, subentry, see, defn  in self.table.index_entries:
            self.write_index(index, subentry, see, defn)
    ...
    def start_td(self, table_data, header=False):
        ...
        if header:
            self.buffer.write("\\begin{embolden}")
        return

    def end_td(self, table_data, header=False):
        if header:
            self.buffer.write("\\end{embolden}")
    ...
    def start_listoffigures(self, list_of_figures):
        self.buffer.write(r"\listoffigures{}")
            #"\\begin{minipage}[t]{1\\linewidth}\\listoffigures\\end{minipage}")
        return
        ...
        try:
            ...
            if rank_num is None:
                if template is None:
                    ...
                    self.buffer.write(f"{name}[{template}]")
            else:
                if template is None:
                    self.buffer.write(f"{name} {rank_num}")
                else:
                    self.buffer.write(f"{name}[{template}] {rank_num}")
    ...
    # def start_sidebar(self, _):
    #     self.buffer.write(r"\begin{mdsidebar}")
    #     return

    # def end_sidebar(self, _):
    #     self.buffer.write(r"\end{mdsidebar}")
    #     return
    ...
    def start_sidebar(self, _):
        ...
        self.buffer.write(
            r"\begin{figure}[t]"            
            r"\colorbox{sidebarcolor}{"
            #r"\begin{lrbox}{\sidebarbox}"
    ...
    def end_sidebar(self, _):
        self.buffer.write(
            r"}"
            r"\end{minipage}"
            #r"\end{lrbox}\fbox{\usebox{\sidebarbox}}"
            r"}"
            r"\end{figure}")
        return
    ...
    def _start_flatcheck(self, flat_check_element):
        """
        Shared formatting for all the flat checks, e.g. <flat13/>//
        
        """
        tag = flat_check_element.tag
        match_obj = _flatcheck_regex.search(tag)
        if match_obj is not None:
            dc = match_obj[0]
            self.buffer.write(fr"\nofatediesymbol/{dc}")
        else:
            raise Exception(
                f"Unknown flat check! {node_to_string(flat_check_element)}")

    start_flat3, end_flat3 = _start_flatcheck, no_op
    start_flat5, end_flat5 = _start_flatcheck, no_op
    start_flat7, end_flat7 = _start_flatcheck, no_op
    start_flat9, end_flat9 = _start_flatcheck, no_op
    start_flat11, end_flat11 = _start_flatcheck, no_op
    start_flat13, end_flat13 = _start_flatcheck, no_op
    start_flat15, end_flat15 = _start_flatcheck, no_op
    start_flat17, end_flat17 = _start_flatcheck, no_op
    start_flat19, end_flat19 = _start_flatcheck, no_op
    start_flat21, end_flat21 = _start_flatcheck, no_op
    start_flat23, end_flat23 = _start_flatcheck, no_op
    start_flat25, end_flat25 = _start_flatcheck, no_op
    start_flat27, end_flat27 = _start_flatcheck, no_op
